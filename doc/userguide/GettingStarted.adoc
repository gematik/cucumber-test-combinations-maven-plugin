...
This will be stored at `./target/generated-combine/countExecutions.<format>`. The export could be delivered in different formats <txt,json>. This could be configured via `countExecutionsFormat` comma separated. If this report is not desired it could be turned of via `countExecutions` set to false.