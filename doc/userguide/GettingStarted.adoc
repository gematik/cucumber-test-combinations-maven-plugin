...
The `SoftFilter`-Feature allows to create `Filter` that will not apply if the requested size of the table could not be generated. First the plugin tries to fill the table with the filter apply and only if the table doesn't have sufficient entries it will handle the generation without all `SoftFilter`
